...
import { 计算数值, 计算最大攻击, 计算最大生命, 计算最大逃跑, 计算最大防御 } from '../方法函数/属性计算';
...
最大连胜次数：${存档.其他.网吧最大连胜次数}次`;
    ...
    const 吃饭信息 = `======吃饭信息======
吃果子次数：${存档.使用次数.果子}
吃果子恢复健康数：${存档.其他.吃果子增加健康数}（综合概率${(存档.其他.吃果子增加健康数 / 存档.使用次数.果子).toFixed(1)}%）
吃熟肉次数：${存档.使用次数.熟肉}次
吃巨无霸次数：${存档.使用次数.巨无霸}
吃炒坚果次数：${存档.使用次数.炒坚果}
吃剩饭次数：${存档.使用次数.剩饭}`;
