...
export class 面板 extends Component {
  ...
  渲染面板列表() {
    ...
    for (const 天赋名 in 存档.天赋) {
      if (存档.天赋[天赋名]) {
        ...
        if (天赋) {
          ...
          index++;
          if (天赋名 === '苦肉计') {
            const 已使用 = !!存档.当日加成.苦肉计全属性;
            创建普通文字(
              this.内容,
              已使用 ? '<今日已使用苦肉计>' : '<点击使用苦肉计：健康-1，当日攻防血+15%>',
              index,
              已使用 ? Color.GRAY : Color.YELLOW,
              () => {
                if (存档.当日加成.苦肉计全属性) {
                  播放文本(this.标签, '今天已经使用过苦肉计！');
                  return;
                }
                if (存档.健康 <= 1) {
                  播放文本(this.标签, '健康不足，无法使用苦肉计！');
                  return;
                }
                存档.健康 -= 1;
                存档.当日加成.苦肉计全属性 = 15;
                保存存档();
                播放文本(this.标签, '苦肉计发动！健康-1，当日攻防血+15%，拼命架势会更加凶狠');
                this.刷新内容列表();
              },
            );
            index++;
    ...
    // 江湖信息
    const 门派熟悉度 = 存档.临时数据.门派熟悉度 || {};
    const 门派声望 = 存档.临时数据.门派声望 || {};
    if (对象求和(门派熟悉度) > 0 || 对象求和(门派声望) > 0) {
      创建普通文字(this.内容, '======江湖信息======', index);
      index++;
      const 江湖信息 = `青竹门  熟悉度${门派熟悉度.青竹门 || 0}  门派声望${门派声望.青竹门 || 0}
铁衣帮  熟悉度${门派熟悉度.铁衣帮 || 0}  门派声望${门派声望.铁衣帮 || 0}
玄水阁  熟悉度${门派熟悉度.玄水阁 || 0}  门派声望${门派声望.玄水阁 || 0}`;
      创建普通文字(this.内容, 江湖信息, index);
      index++;
    }
    // 残页
