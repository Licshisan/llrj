...
import { 剧情项目类型, 获取剧情 } from "../内容加载/剧情";
...
export class 剧情 extends Component {
	...
	触发剧情(剧情名称: string) {
		...
		选项一.getChildByName("标签").getComponent(Label).string = this.剧情.选项一;
		选项一.targetOff(this)
		选项一.on(Button.EventType.CLICK, () => this.剧情.结果一(this.上下文), this);
		选项一.active = !!this.剧情.选项一;


		选项二.getChildByName("标签").getComponent(Label).string = this.剧情.选项二;
		选项二.targetOff(this)
		选项二.on(Button.EventType.CLICK, () => this.剧情.结果二(this.上下文), this);
		选项二.active = !!this.剧情.选项二;

		const sequence = tween(this.node).delay(1 / 设置.播放速度);
		for (let i = 0; i < this.剧情.文本.length; i++) {
			sequence.call(() => 创建动画文字(this.文本容器, this.剧情.文本[i], i)).delay(2 / 设置.播放速度);
		}
