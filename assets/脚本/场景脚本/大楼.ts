...
export class 大楼 extends Component {
	...
	start() {
		...
		if (Math.random() * 100 < 70) {
			...
			一层按钮.on(Button.EventType.CLICK, () => 播放文本(this.标签, "“放假啦！哈哈哈哈哈！”"), this);
		}
		...
		if (Math.random() * 100 < 70) {
			...
			二层按钮.on(Button.EventType.CLICK, () => 播放文本(this.标签, "“放假啦！哈哈哈哈哈！”"), this);
		}
		...
		if (Math.random() * 100 < 70) {
			...
			三层按钮.on(Button.EventType.CLICK, () => 播放文本(this.标签, "“放假啦！哈哈哈哈哈！”"), this);
		}
		...
		if (Math.random() * 100 < 计算数值("网吧开门概率", 70)) {
			...
			四层按钮.on(Button.EventType.CLICK, () => 播放文本(this.标签, "“放假啦！哈哈哈哈哈！”"), this);
		}
		...
		if (Math.random() * 100 < 50) {
			...
			五层按钮.on(Button.EventType.CLICK, () => 播放文本(this.标签, "“放假啦！哈哈哈哈哈！”"), this);
		}


		this.大楼.getChildByName("按钮容器").getChildByName("选择按钮6").on(Button.EventType.CLICK, () => 播放文本(this.标签, "广告位招租！"), this);
		// 餐厅界面
		this.餐厅.getChildByName("返回按钮").on(Button.EventType.CLICK, () => {
			缩小消失(this.餐厅);
			放大出现(this.大楼);
		}, this);

		// 巨无霸
		this.餐厅.getChildByName("按钮容器").getChildByName("选择按钮1").on(Button.EventType.CLICK, () => this.点击巨无霸(), this);
		// 炒坚果
		this.餐厅.getChildByName("按钮容器").getChildByName("选择按钮2").on(Button.EventType.CLICK, () => this.点击炒坚果(), this);
	}
	点击一楼() {
		if (存档.其他.金融赢钱率 === 0) {
		...
		if (存档.金钱 > 500) {
			播放文本(this.标签, '"不好意思，你已超过国家监管限定金额，请你去实体银行吧，我们只是网上的小银行~"');
			return
		}
		...
		if (Math.random() * 100 < 存档.其他.金融赢钱率) {
			...
			if(存档.其他.投资已挽回次数 < 投资挽回次数){
				存档.其他.投资已挽回次数 ++
				播放文本(this.标签, `投资失败...【投资失败笔记】已挽回一次投资失败金额（${存档.其他.投资已挽回次数}/${投资挽回次数}），本次投资无损失`);
				return
			}

			存档.金钱 -= 赢钱数;
			if(Math.random() * 100 < 1){
				播放文本(this.标签, `投资失败...金钱缩水${赢钱率.toFixed(1)}%(-${(赢钱数 / 10).toFixed(1)})，你汲取了失败地教训，获得藏品【投资失败笔记】*1`);
				return
			} 
		...
		存档.其他.金融赢钱率 = 0;
		this.大楼.getChildByName("按钮容器").getChildByName("选择按钮1").getChildByName("标签").getComponent(Label).string = "一楼：晓风金融（今日已结束）"
		this.更新();
	}
	点击二楼() {
		const 商品表 = { 晓风披肩: 100, 小裤裤: 20, 晓月手链: 20, 幸运石: 20, 板砖: 20, 滑稽裤: 20, };
		...
		this.大楼.getChildByName("按钮容器").getChildByName("选择按钮2").getChildByName("标签").getComponent(Label).string = "二楼：晓风服饰（已售罄）"
		this.更新();
	}
	...
	更新() {
		this.属性.getComponent(Label).string = `金钱 ${(存档.金钱 / 10).toFixed(1)}\n饥饿 ${存档.饥饿}/${计算最大饥饿()}`;
		保存存档()
	}
