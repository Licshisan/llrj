...
export class 睡觉 extends Component {
  ...
  恢复() {
    ...
    const 精力恢复 = 计算数值('睡觉恢复精力', 最大精力 - 存档.精力);
    const 饥饿消耗 = 计算数值('睡觉消耗饥饿', 20);
    const 理论生命恢复 = 计算数值('睡觉恢复生命', 0);
    const 生命恢复 = 存档.天赋.超量恢复 >= 2
      ? 理论生命恢复
      : Math.min(理论生命恢复, Math.max(最大生命 - 存档.生命, 0));
    自动进食();
    自动进食();
    自动进食();
    ...
    存档.饥饿 -= 饥饿消耗;
    存档.生命 += 生命恢复;
    if (存档.天赋.超量恢复 < 2 && 存档.生命 > 最大生命) 存档.生命 = 最大生命;
    ...
    this.属性容器.getChildByName('精力').getChildByName('标签').getComponent(Label).string =
      `精力 +${精力恢复}（${存档.精力}/${最大精力}）`;
    this.属性容器.getChildByName('饥饿').getChildByName('标签').getComponent(Label).string =
      `饥饿 -${饥饿消耗}（${存档.饥饿}/${最大饥饿}）`;
    this.属性容器.getChildByName('生命').getChildByName('标签').getComponent(Label).string =
      `生命 +${生命恢复}（${存档.生命}/${最大生命}）`;
  }
  ...
  结算() {
    ...
    if (Math.random() * 100 < 20) {
      ...
      if (
        获取地区名称() === '省城' &&
        Math.random() * 100 < (存档.物品.枪 - 1) * 10 + 1 &&
        存档.物品.枪 > 0
      ) {
        存档.当前敌人 = '陈晓（大大）2';
      }
    ...
    if (Math.random() * 100 < 存档.罪恶 / 3) {
      ...
      if (存档.天赋.失眠症 >= 1) {
        存档.状态.疲惫 = 1;
        结果文本.push('【失眠】精力-50%！因失眠症获得【疲惫】状态，前进/探索消耗精力+1');
      } else {
        结果文本.push('【失眠】精力-50%！');
      }
    }
    ...
    if (存档.临时数据.房源) {
      if (存档.其他.住房已居住天数 < 存档.其他.住房天数) {
        ...
        if (房源.中介名称 === '刘老头' && Math.random() * 100 < 25) {
          ...
          结果文本.push(
            `【${房源.名称}】充分的休息让你额外恢复${恢复精力}精力（${(房源.恢复精力 * 100).toFixed(2)}%），${恢复健康}点健康，已居住${存档.其他.住房已居住天数}/${存档.其他.住房天数}天`,
          );
        }
      } else {
        结果文本.push(
          `【房租已到期】已居住${存档.其他.住房已居住天数}/${存档.其他.住房天数}天，你回到了桥洞休息`,
        );
        存档.其他.住房已居住天数 = 0;
        存档.其他.住房天数 = 0;
        存档.临时数据.房源 = 0;
