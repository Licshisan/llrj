...
export class 存档 extends Component {
  ...
  加载存档() {
    ...
    存档名称列表.forEach((存档) => {
      ...
      存档项目节点.getChildByName('选择按钮').getChildByName('标签').getComponent(Label).string =
        `${this.计算地区名称(存档)}.${存档.天数}天 【${存档.套餐名称}】 ${存档.距离}km`;
      存档项目节点.getChildByName('标签').getComponent(Label).string =
        `创建时间：${格式化日期字符串(存档.创建时间)}`;
      存档项目节点
        .getChildByName('按钮容器')
        .getChildByName('黑色按钮1')
        .getChildByName('标签')
        .getComponent(Label).string = '删  除';
      存档项目节点
        .getChildByName('按钮容器')
        .getChildByName('黑色按钮2')
        .getChildByName('标签')
        .getComponent(Label).string = '进  入';

      存档项目节点
        .getChildByName('按钮容器')
        .getChildByName('黑色按钮1')
        .on(Button.EventType.CLICK, () => this.点击删除(存档.存档名称), this);
      存档项目节点
        .getChildByName('按钮容器')
        .getChildByName('黑色按钮2')
        .on(Button.EventType.CLICK, () => this.点击进入(存档.存档名称), this);
    });
