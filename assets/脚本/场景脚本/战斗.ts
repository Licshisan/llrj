...
export class 战斗 extends Component {
    ...
    进入PVP战斗(敌人名称: string) {
        ...
        if(敌人名称 === '时空流浪者'){
            ...
            敌人设置 = globalThis?.PVP玩家数据?.设置
        } 
        else if(敌人名称 === '另一个自己'){
            敌人存档数据 = 存档
            敌人显示名称 = `另一个自己`
            敌人设置 = JSON.parse(JSON.stringify(设置));
        }  
        else{
        ...
        const 物品1 = 物品表1.splice(Math.random()*物品表1.length|0, 1)[0];
        const 物品2 = 物品表2.splice(Math.random()*物品表2.length|0, 1)[0];
        ...
        const 出场语 = 敌人设置?.其他?.出场语 || `是你要挑战我${敌人显示名称}吗？`
        const 战胜语 = 敌人设置?.其他?.战胜语 || "小弟弟也不过如此啦~"
        const 战败语 = 敌人设置?.其他?.战败语 || "小弟甘拜下风！"
        ...
        const 战斗敌人: 战斗角色 = {
            ...
            最大生命: 最大生命,
            逃跑: Math.min(执行存档上下文(敌人存档, () => 计算最大逃跑()) / 100, 30),
