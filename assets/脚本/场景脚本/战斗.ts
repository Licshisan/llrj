...
import { 保存存档, 存档, 创建默认值代理, 执行存档上下文 } from "../管理器/存档管理器";
import { 从0放大缩小, 放大出现, 缩小消失, 震动 } from "../方法函数/动画效果";
import { 主页 } from "./主页";
import { 执行钩子 } from "../管理器/钩子管理器";
import { 计算最大压制, 计算最大攻击, 计算最大生命, 计算最大防御, 计算最大逃跑, 计算数值 } from "../方法函数/属性计算";
...
export class 战斗 extends Component {
    ...
    进入PVP战斗(敌人名称: string) {
        ...
        let 敌人显示名称 = ""
        let extInfo = null
        if(敌人名称 === '时空流浪者' && 存档.临时数据.时空流浪者){
            敌人存档数据 = 存档.临时数据.时空流浪者.save
            敌人显示名称 = 存档.临时数据.时空流浪者.player.名称 || "时空流浪者"
            extInfo = 存档.临时数据.时空流浪者.player.扩展信息
        } else if(敌人名称 === '镜像人' || 敌人名称 === '另一个自己'){
            敌人存档数据 = 存档
            敌人显示名称 = 敌人名称
            extInfo = 玩家.扩展信息
        } else{
            this.结束战斗(`PVP加载失败！【${敌人名称}】（请反馈开发者）`)
            return;
        }
        if (!敌人存档数据 || typeof 敌人存档数据 !== 'object') {
            this.结束战斗(`PVP加载失败！【${敌人名称}】`)
            return;
        }
        ...
        try {
            ...
            console.error('PVP敌人存档数据深拷贝失败:', e);
            深拷贝数据 = {};
            this.结束战斗(`PVP加载失败！【${敌人名称}】`)
            return;
        }
        const 敌人存档: typeof 存档 = 创建默认值代理(深拷贝数据);
        // const 物品表1 = [ "果子", "木材", "易拉罐", "生肉", "亚麻", "艾草", "烟草", "烟头", "黑曜石", "火狐皮", "50ml啤酒", "白色粉末", "眼泪" ]
        // const 物品表2 = [ "熟肉", "伤药", "帐篷", "木棍", "麻布衣", "陷阱", "驱蚊工具", "香烟", "匕首", "皮衣", "黑刀", "红夹克", "啤酒", "┑(=^ω^=)┑", "子弹", "晓风披肩", "漂亮石头", "放大镜", "装避鞋", "枪", "板砖", "小裤裤", "女装", "创口贴", "滑稽裤", "护身符", "幸运石", "晓月手链" ]
        // todo 掉落物
        const 掉落物列表: 概率类型[] = [];
        ...
        const  执行敌人技能视角 = (敌人存档: any, 对局: 对局类型, 钩子名称: string) => {
            执行存档上下文(敌人存档, () => {
                const 原主角 = 对局.主角;
                const 原敌人 = 对局.敌人;
                对局.主角 = 原敌人;
        ...
        const 出场语 = extInfo?.出场语 || `是你要挑战我${敌人显示名称}吗？`
        const 战胜语 = extInfo?.战胜语 || "小弟弟也不过如此啦~"
        const 战败语 = extInfo?.战败语 || "小弟甘拜下风！"
    ...
    点击攻击() {
        ...
        this.对局.攻击 = { 初始值: this.对局.主角.攻击, 基础加成: 0, 加法乘率: 0, 独立乘区: 1, 最终修正: 0, 计算结果: 0 }
        this.对局.防御 = { 初始值: this.对局.敌人.防御, 基础加成: 0, 加法乘率: 0, 独立乘区: 1, 最终修正: 0, 计算结果: 0 }
        this.对局.伤害 = { 初始值: 0, 基础加成: 0, 加法乘率: 0, 独立乘区: 1, 最终修正: 0, 计算结果: 0 }
        this.对局.结果文本 = [];
        this.对局.主角.攻击前(this.对局)
        this.对局.敌人.被攻击前(this.对局)
        存档.生命 = this.对局.主角.生命
        this.对局.攻击.计算结果 = Math.max((this.对局.攻击.初始值 + this.对局.攻击.基础加成) * (1 + this.对局.攻击.加法乘率) * this.对局.攻击.独立乘区, 0) + this.对局.攻击.最终修正
        this.对局.防御.计算结果 = Math.max((this.对局.防御.初始值 + this.对局.防御.基础加成) * (1 + this.对局.防御.加法乘率) * this.对局.防御.独立乘区, 0) + this.对局.防御.最终修正
        this.对局.伤害.初始值 = Math.max(this.对局.攻击.计算结果 - this.对局.防御.计算结果, 0);
        this.对局.主角.攻击时(this.对局)
        this.对局.敌人.被攻击时(this.对局)
        存档.生命 = this.对局.主角.生命
        this.对局.伤害.计算结果 = Math.max((this.对局.伤害.初始值 + this.对局.伤害.基础加成) * (1 + this.对局.伤害.加法乘率) * this.对局.伤害.独立乘区, 0) + this.对局.伤害.最终修正
        ...
        this.对局.结果文本.push(`${this.对局.敌人.名称}受到${Math.floor(this.对局.伤害.计算结果)}点伤害。`)
    ...
    敌人攻击() {
        ...
        this.对局.攻击 = { 初始值: this.对局.敌人.攻击, 基础加成: 0, 加法乘率: 0, 独立乘区: 1, 最终修正: 0,  计算结果: 0, }
        this.对局.防御 = { 初始值: this.对局.主角.防御, 基础加成: 0, 加法乘率: 0, 独立乘区: 1, 最终修正: 0, 计算结果: 0, }
        this.对局.伤害 = { 初始值: 0, 基础加成: 0, 加法乘率: 0, 独立乘区: 1,  最终修正: 0, 计算结果: 0,  }
        this.对局.结果文本 = [];
        ...
        this.对局.结果文本.push(`${this.对局.主角.名称}受到${Math.floor(this.对局.伤害.计算结果)}点伤害。`)
