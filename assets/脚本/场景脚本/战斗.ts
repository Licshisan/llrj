...
export class 战斗 extends Component {
    ...
    进入PVP战斗(敌人名称: string) {
        ...
        let 敌人显示名称 = ""
        if(敌人名称 === '时空流浪者'){
            敌人存档数据 = globalThis?.PVP玩家数据?.存档
            敌人显示名称 = globalThis?.PVP玩家数据?.昵称 || "时空流浪者"
        } 
        else if(敌人名称 === '另一个自己'){
            敌人存档数据 = 存档
            敌人显示名称 = `另一个自己`
        ...
        const 可用物品列表 = Object.entries(敌人存档.物品 || {}).map(([n]) => n);
        const 物品1 = 可用物品列表.splice(Math.random()*可用物品列表.length|0, 1)[0];
        const 物品2 = 可用物品列表.length ? 可用物品列表.splice(Math.random()*可用物品列表.length|0, 1)[0] : null;

        const 掉落物列表: 概率类型[] = [];
        掉落物列表.push({
            名称: 物品1,
            概率: 100,
            数量: Math.random() < 0.5 ? 1 : 2
        });
        if(存档.天数 > 150){
            掉落物列表.push({
                名称: 物品2,
        ...
        const 战斗敌人: 战斗角色 = {
            名称: 敌人名称,
            显示名称: 敌人显示名称,
            等级: Math.floor(敌人存档.经验 / 10),
            生命: 敌人显示名称 == '另一个自己' ? 最大生命 : 敌人存档.生命,
            方法: "普攻",
            攻击: 执行存档上下文(敌人存档, () => 计算最大攻击()),
            防御: 执行存档上下文(敌人存档, () => 计算最大防御()),
            最大生命: 最大生命,
            逃跑: 执行存档上下文(敌人存档, () => 计算最大逃跑()) - 100,
            压制: 执行存档上下文(敌人存档, () => 计算最大压制()) + 100,
            增加声望: 3,
            增加属性: 2,
            损失健康: 敌人显示名称 == '另一个自己' ? 10 : 1,
            出场语: `【${敌人显示名称}】前来挑战！`,
            掉落物: 掉落物列表,
            战斗初始化: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('战斗初始化', [对局]));
            },
            攻击前: (对局: 对局类型) => {
