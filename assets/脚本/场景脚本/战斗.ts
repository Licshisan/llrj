...
export class 战斗 extends Component {
    ...
    进入PVP战斗(敌人存档数据: any) {
        ...
        const 可用物品列表 = Object.entries(敌人存档.物品 || {})
            .filter(([_, 数量]) => 数量 && 数量 > 0)
            .map(([名称]) => 名称);

        const 掉落物列表: 概率类型[] = [];
        const 抽取数量 = Math.random() < 0.5 ? 1 : 2;
        
        for (let i = 0; i < Math.min(抽取数量, 可用物品列表.length); i++) {
        ...
        const 战斗敌人: 战斗角色 = {
            ...
            攻击前: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('攻击前', [对局]));
            },
            攻击时: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('攻击时', [对局]));
            },
            攻击后: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('攻击后', [对局]));
            },

            被攻击前: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('被攻击前', [对局]));
            },
            被攻击时: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('被攻击时', [对局]));
            },
            被攻击后: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('被攻击后', [对局]));
            },

            失败效果: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('战斗失败', [对局]));
            },
            胜利效果: (对局: 对局类型) => {
                执行存档上下文(敌人存档, () => 执行钩子('战斗胜利', [对局]));
            },
            其他: {
                存档: 敌人存档,
                技能点数: 0,
