...
export class 战斗 extends Component {
    ...
    进入PVP战斗(敌人名称: string) {
        ...
        if (!敌人存档数据 || typeof 敌人存档数据 !== 'object') {
            console.error('PVP敌人存档数据无效:', 敌人存档数据);
            return;
        }
        let 深拷贝数据;
        try {
        ...
        const 敌人存档 = 创建默认值代理(深拷贝数据);

        const 可用物品列表 = Object.entries(敌人存档.物品 || {}).map(([名称]) => 名称);
        const 掉落物列表: 概率类型[] = [];
        const 随机索引 = Math.floor(Math.random() * 可用物品列表.length);
        const 物品名称 = 可用物品列表.splice(随机索引, 1)[0];
        掉落物列表.push({
            名称: 物品名称,
            概率: 100,
        ...
        const 战斗敌人: 战斗角色 = {
            名称: 敌人存档名称,
            显示名称: 敌人存档名称,
            等级: Math.floor(敌人存档.经验 / 100),
            生命: 敌人存档名称 == '另一个自己' ? 最大生命 : 敌人存档.生命,
            方法: "普攻",
            攻击: 执行存档上下文(敌人存档, () => 计算最大攻击()),
            防御: 执行存档上下文(敌人存档, () => 计算最大防御()),
            最大生命: 最大生命,
            逃跑: 执行存档上下文(敌人存档, () => 计算最大逃跑()),
