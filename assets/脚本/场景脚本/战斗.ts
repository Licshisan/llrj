...
export class 战斗 extends Component {
    ...
    进入战斗(敌人名称: string) {
        ...
        const 战斗主角: 战斗角色 = {
            ...
            攻击前: (对局: 对局类型) => {
                const 攻击前 = 执行钩子('攻击前', [对局])
                结果文本.push(...攻击前)
            },
            攻击时: (对局: 对局类型) => {
                const 攻击时 = 执行钩子('攻击时', [对局])
                结果文本.push(...攻击时)
            },
            攻击后: (对局: 对局类型) => {
                const 攻击后 = 执行钩子('攻击后', [对局])
                结果文本.push(...攻击后)
            },

            被攻击前: (对局: 对局类型) => {
               const 被攻击前 = 执行钩子('被攻击前', [对局])
                结果文本.push(...被攻击前)
            },
            被攻击时: (对局: 对局类型) => {
                const 被攻击时 = 执行钩子('被攻击时', [对局])
                结果文本.push(...被攻击时)
            },
            被攻击后: (对局: 对局类型) => {
                const 被攻击后 = 执行钩子('被攻击后', [对局])
                结果文本.push(...被攻击后)
            },

            胜利效果: (对局: 对局类型) => {
                const 战斗胜利 = 执行钩子('战斗胜利', [对局])
                结果文本.push(...战斗胜利)
            },
            失败效果: (对局: 对局类型) => {
                const 战斗失败 = 执行钩子('战斗失败', [对局])
                结果文本.push(...战斗失败)
            },
            其他: {}
        }
    ...
    敌人攻击() {
        ...
        // 敌人攻击前
        const 敌人攻击前文本 = this.对局.敌人.攻击前(this.对局)

        执行钩子("被攻击前", [this.对局])


        // 敌人攻击
        const 敌人攻击文本 = this.对局.敌人.攻击时(this.对局)

        执行钩子("被攻击时", [this.对局])

        // 伤害结算
        存档.生命 -= this.对局.伤害.计算结果;
        this.对局.结果文本.unshift(`${this.对局.敌人.名称}使用「${this.对局.敌人.方法}」`)
        this.对局.结果文本.push(`你受到${this.对局.伤害.计算结果}点伤害。`);
