...
function 生成神秘商人列表(商贩实例: 商贩): 商贩项目类型[] {
  const 列表: 商贩项目类型[] = [];
  const 商品池 = [
    {
      概率: 0.8,
    ...
    },
    {
      概率: 0.5,
      商品ID: '暗蚀果实',
      名称: () => `暗蚀果实（3元）——散发不祥气息的果实`,
      价格: 30,
      限购: true,
      购买: () => {
        存档.物品.暗蚀果实 = (存档.物品.暗蚀果实 || 0) + 1;
        return '获得【暗蚀果实】*1！这颗果实散发着不祥的气息...';
      },
    },
    {
      概率: 0.9,
  ...
  商品池.forEach((商品) => {
    if (Math.random() < 商品.概率) {
      列表.push({
        名称: () => {
          if (商品.限购 && 商贩实例.神秘商人已购列表.includes(商品.商品ID)) {
            return '已售罄';
          }
          return typeof 商品.名称 === 'function' ? 商品.名称() : 商品.名称;
        },
        购买: () => {
          if (商品.限购 && 商贩实例.神秘商人已购列表.includes(商品.商品ID)) {
            return '本次交易每人限购一个！';
