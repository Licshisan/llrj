...
export class 图鉴 extends Component {
  ...
  渲染天赋列表() {
    ...
    默认天赋表.forEach((天赋, index) => {
      ...
      if(!天赋.隐藏 || (天赋.隐藏 && 等级 > 0)){
        ...
        title += 天赋.品质
        创建普通文字(this.内容, `【${天赋.名称}(${title})】LV${等级}\n${天赋.说明}`, index, 等级 > 0 ? 天赋.颜色 : null);
      } 
    });
  }
  ...
  渲染特质列表() {
    ...
    创建普通文字(this.内容, `点击可以选择下局生效的特质(你已选择${设置.生效特质.length}/${开局特质数量})`, -1);
    默认特质表.forEach((特质, index) => {
      const 等级 = 计算特质等级(特质.名称)
      const 已选择 = 设置.生效特质.includes(特质.名称)
      创建普通文字(this.内容, `【${特质.名称}】LV${等级}${已选择?"[已选择]" : ''}\n${特质.说明}`, index, 等级 > 0 ? Color.YELLOW : null, () => {
        if(等级 == 0) return
  ...
  渲染技能列表() {
    ...
    默认技能表.forEach((技能, index) => {
      ...
      创建普通文字(this.内容, `【${技能.名称}】LV${等级}\n${技能.描述}`, index, 等级 > 0 ? Color.BLUE : null);
    });
  }
  ...
  渲染藏品列表() {
    ...
    默认藏品表.forEach((藏品, index) => {
      ...
      创建普通文字(this.内容, `【${藏品.名称}】已拥有${数量}\n${藏品.描述}`, index, 数量 > 0 ? Color.GREEN : null);
    });
  }
}