  ...
  生成天赋摘要文本() {
    const 已选天赋文本 =
      设置.锁定天赋
        ...
        .map((天赋) => (天赋 ? `- ${天赋.名称}(${天赋.隐藏 ? '隐藏·' : ''}${天赋.品质})` : ''))
        .filter(Boolean)
        .join('\n') || '- 暂未选择';

    const 难度消耗文本 = [ '普通', '试炼', '真实', '残酷', '绝境' ]
      .map((难度) => {
        const 状态 = 计算自选天赋点状态(设置.锁定天赋, 难度);
        return `- ${难度}：消耗${状态.消耗}点，剩余${状态.剩余}点，${状态.可以进入 ? '可以进入' : '不可进入'}`;
      })
      .join('\n');

    const 状态 = 计算自选天赋点状态(设置.锁定天赋, '普通');
    return `======天赋自选======
天赋点：基础${状态.基础天赋点}点 + 额外${状态.额外天赋点}点 = 共${状态.基础天赋点 + 状态.额外天赋点}点
已选择：${设置.锁定天赋.length}/${状态.锁定上限}
...
==================`;
  ...
  生成天赋条目文本(天赋) {
    ...
    return {
      等级,
      已锁定,
      文本: `【${天赋.名称}(${天赋.隐藏 ? '隐藏·' : ''}${天赋.品质})】${已锁定 ? '【已选择】' : ''}\n${天赋.说明}`,
      颜色: 等级 > 0 ? 天赋.颜色 : null,
    };
  }

  重新渲染天赋摘要() {
    const 旧节点 = this.天赋摘要节点;
    const 兄弟序号 = 旧节点?.isValid ? 旧节点.getSiblingIndex() : -1;
    if (旧节点?.isValid) 旧节点.destroy();
    ...
    if (兄弟序号 >= 0) 新节点.setSiblingIndex(兄弟序号);
    this.天赋摘要节点 = 新节点;
  }

  重新渲染单个天赋条目(天赋名称: string) {
    const 旧节点 = this.天赋条目节点表[天赋名称];
    const 天赋 = 默认天赋表.find((t) => t.名称 === 天赋名称);
    if (!旧节点?.isValid || !天赋) return;

    const 兄弟序号 = 旧节点.getSiblingIndex();
    旧节点.destroy();
    delete this.天赋条目节点表[天赋名称];

    const 新节点 = this.创建天赋条目节点(天赋);
    新节点.setSiblingIndex(兄弟序号);
    this.天赋条目节点表[天赋名称] = 新节点;
  }

  创建天赋条目节点(天赋) {
    const { 等级 } = this.生成天赋条目文本(天赋);
    const 节点 = 创建普通文字(
      this.内容,
      this.生成天赋条目文本(天赋).文本,
      等级 > 0 ? 天赋.颜色 : null,
      () => {
        const 当前是否已选 = 设置.锁定天赋.includes(天赋.名称);
        if (等级 == 0 && !当前是否已选) return;
        if (!当前是否已选) {
          const 状态 = 计算自选天赋点状态(设置.锁定天赋, '普通');
          if (设置.锁定天赋.length + 1 > 状态.锁定上限) return;
