...
export class 图鉴 extends Component {
  ...
  渲染天赋列表() {
    ...
    const 占用普通难度天赋点 = 设置.锁定天赋.reduce((sum, name) => {
      ...
      if (天赋) {
        const 花费 = 天赋.负面 ? 1 : 天赋.品质 === '普通' ? 1 : 天赋.品质 === '稀有' ? 2 : 天赋.品质 === '史诗' ? 3 : 天赋.品质 === '传说' ? 4 : 1;
        return sum + 花费;
      }
      return sum;
    }, 0);
    const 占用试炼难度天赋点 = 设置.锁定天赋.reduce((sum, name) => {
      const 天赋 = 默认天赋表.find((t) => t.名称 === name);
      if (天赋) {
        const 花费 = 天赋.负面 ? 2 : 天赋.品质 === '普通' ? 2 : 天赋.品质 === '稀有' ? 3 : 天赋.品质 === '史诗' ? 4 : 天赋.品质 === '传说' ? 5 : 1;
        return sum + 花费;
      }
      return sum;
    }, 0);
    const 占用真实难度天赋点 = 设置.锁定天赋.reduce((sum, name) => {
      const 天赋 = 默认天赋表.find((t) => t.名称 === name);
      if (天赋) {
        const 花费 = 天赋.负面 ? 3 : 天赋.品质 === '普通' ? 3 : 天赋.品质 === '稀有' ? 4 : 天赋.品质 === '史诗' ? 5 : 天赋.品质 === '传说' ? 6 : 1;
        return sum + 花费;
      }
      return sum;
    }, 0);
    const 占用残酷难度天赋点 = 设置.锁定天赋.reduce((sum, name) => {
      const 天赋 = 默认天赋表.find((t) => t.名称 === name);
      if (天赋) {
        const 花费 = 天赋.负面 ? 4 : 天赋.品质 === '普通' ? 4 : 天赋.品质 === '稀有' ? 5 : 天赋.品质 === '史诗' ? 6 : 天赋.品质 === '传说' ? 7 : 1;
        return sum + 花费;
      }
      return sum;
    }, 0);
    const 占用绝境难度天赋点 = 设置.锁定天赋.reduce((sum, name) => {
      const 天赋 = 默认天赋表.find((t) => t.名称 === name);
      if (天赋) {
        const 花费 = 天赋.负面 ? 5 : 天赋.品质 === '普通' ? 5 : 天赋.品质 === '稀有' ? 6 : 天赋.品质 === '史诗' ? 7 : 天赋.品质 === '传说' ? 8 : 1;
        return sum + 花费;
      }
      return sum;
    }, 0);
...
2.天赋花费点数：负面1，普通1，稀有2，史诗3，传说4
3.你最多可以锁定${锁定上限}个正面或负面天赋
4.不同难度每个天赋额外花费：试炼1，真实2，残酷3，绝境4
5.新建游戏所选的难度对应的天赋点数不能超过当前所有的天赋点数
6.每次进入游戏不消耗基础天赋点，所选天赋占用点数超出基础天赋点的部分会消耗额外天赋点
7.通过完成成就等途径可以获得基础天赋点数
8.通过通关游戏可以获得累计的额外天赋点数
9.你当前已选择天赋：
