...
export class 主页 extends Component {
    ...
    点击结局() {
        ...
        if(!存档.剧情.愿意回家){
            if (存档.伙伴.晓月关系) {
                存档.当前剧情 = "晓月结局"
            } else if (存档.伙伴.碧瑶关系 && 存档.伙伴.碧瑶好感 >= 5000 && 存档.剧情.击败阿胜 && 存档.剧情.通关 && 得分 >= 60) {
                存档.当前剧情 = "碧瑶同行"
    ...
    点击睡觉() {
        if (存档.剧情.住在桥洞) {
            ...
            if (存档.精力 > 0 || 设置.防误触) {
                const 提示文本 = 存档.精力 > 0 ? "系统检测到你还有剩余的精力，是否要睡觉？" : "防误触提示：是否要睡觉？"
                globalThis.确认参数 = {
                    文本: 提示文本,
                    按钮: {
                        "确认睡觉": () => {
    ...
    探索条件() {
        ...
        // 比武大会
        if (存档.当前地点 === '市中心' && 存档.停留天数.省城 >= 30 && 存档.停留天数.省城 <= 36 && !存档.其他.比武大会结束) {
        ...
        // 地下入口
        if(存档.当前地点 === '城中村' && 存档.停留天数.省城 < 8 && !存档.其他.解锁地下竞技场){
        ...
        // 城中村住房
        if(存档.当前地点 === '城中村' && 存档.停留天数.省城 > 8 && 计算技能等级("世界感知") >= 2 && Math.random() * 100 < 1){
    ...
    主要逻辑() {
        ...
        let 前进探索战斗权重 = 计算数值('前进探索战斗权重', 前进探索战斗初始权重)
        let 前进探索事件权重 = 计算数值('前进探索事件权重', 前进探索事件初始权重)
        let 前进探索收集权重 = 计算数值('前进探索收集权重', 前进探索收集默认权重)
