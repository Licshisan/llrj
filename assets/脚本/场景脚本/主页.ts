...
export class 主页 extends Component {
  ...
  主动技能() {
    ...
    this.苦肉按钮.on(Button.EventType.CLICK, () => {
      if(存档.天赋["死侍"]){
        ...
        存档.其他.苦肉发动次数 ++
        this.更新()
        this.播放文本(`【苦肉发动，健康-1，当日攻防血属性+4%！今日已触发${存档.其他.苦肉发动次数}次，全属性提升${存档.其他.苦肉发动次数 * 4}%`);
      }
    }, this);
  }
  ...
  探索条件() {
    ...
    // 地下入口
    if (存档.当前地点 === '城中村' && 存档.停留天数.省城 < 12 && !存档.其他.解锁地下竞技场) {
    ...
    // 江湖人士
    if (存档.技能.世界感知 >= 2 && (存档.游戏难度 === '真实' || 存档.游戏难度 === '残酷' || 存档.游戏难度 === '绝境')) {
      if (获取地区名称() === '县城' && Math.random() * 100 < 3) {
        this.基本消耗();
