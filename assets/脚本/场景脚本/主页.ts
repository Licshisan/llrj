...
export class 主页 extends Component {
  ...
  主动技能() {
    ...
    this.苦肉按钮.on(
      ...
      () => {
        if (存档.天赋['死侍']) {
          const 单次提升 = 存档.天赋['死侍'] >= 3 ? 4 : 2;
          存档.健康 -= 1;
          存档.其他.苦肉发动次数++;
          this.更新();
          this.播放文本(
            `【苦肉发动，健康-1，当日攻防血属性+${单次提升}%！今日已触发${存档.其他.苦肉发动次数}次，全属性提升${存档.其他.苦肉发动次数 * 单次提升}%`,
          );
        }
      },
      this,
  ...
  山洞特殊探索(扫荡模式 = false): string {
    ...
    const 结果文本 = 拾取结果[Math.floor(Math.random() * 拾取结果.length)]();
  ...
  主要逻辑(扫荡模式 = false): { 是搜索事件: boolean; 结果文本: string } {
    ...
    if (随机数 < 前进探索战斗权重) {
      ...
      const 事件名称 = 抽取项目(事件表);
      存档.其他.随机事件次数++;
      this.node.getComponent(事件).触发事件(事件名称);
