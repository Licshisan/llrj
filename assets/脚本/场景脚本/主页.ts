...
export class 主页 extends Component {
    ...
    游戏结束() {
        if (存档.天数 >= 180 && 存档.剧情.通关) {
            ...
            this.按钮容器.getChildByName("前进").getChildByName("标签").getComponent(Label).string = '探索'
    ...
    探索条件() {
        const 计算县城停留天数 = 计算数值("计算县城停留天数", 44)
