import { Node, Label, Color, tween, v3, Vec3, UIOpacity, Sprite, UITransform, find, Tween } from "cc";
...
export function 淡入(目标节点: Node, 持续时长: number = 2.4): void {
	...
	tween(透明组件).to(持续时长 / 设置.播放速度, { opacity: 255 }).start();
}
...
export function 多次闪烁(目标节点: Node): void {
	...
	const blinkAction = () => {
		...
		labelComp.color = new Color(
			255,
			255 - 200 * isBlinkPhase,
			255 - 200 * isBlinkPhase
		);
		blinkCount++;
